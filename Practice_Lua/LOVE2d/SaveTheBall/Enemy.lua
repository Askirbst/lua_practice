...
function Enemy(level)
    ...
    return {
        ...
        EnemyColorR = math.random(0 , 255) / 255,
        EnemyColorG = math.random(0 , 255) / 255,
        EnemyColorB = math.random(0 , 255) / 255,

        shape = math.random(1, 3),
        ...
        draw = function (self, level)
            ...
            love.graphics.setColor(self.EnemyColorR, self.EnemyColorG, self.EnemyColorB)

            if self.shape == 1 then
                love.graphics.circle("fill", self.x, self.y, self.radius)
            elseif self.shape == 2 then
                love.graphics.polygon("fill", self.x, self.y, self.x + 20, self.y + 40, self.x - 20, self.y + 40)
